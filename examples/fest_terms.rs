...
fn main() {
    ...
    for (i, name) in JIE_QI_NAMES.iter().enumerate() {
        ...
        println!(
            "  {}: {}年{}月{}日 {:02}:{:02}:{:02}",
            name, jd.year, jd.month, jd.day, jd.hour as i32, jd.minute as i32, jd.second as i32
        );
    }
    ...
    for day in &month_calendar.days_info {
        ...
        println!(
            "  {}/{}/{} {}月{}",
            year, 1, day.day.d, day.lunar_month_name, day.lunar_day_name
        );
    ...
    // 1. 获取2025年春节（通过月历数据）
    let spring_festival = month_calendar
        .days_info
        .iter()
    ...
    println!(
        "  2025年春节: {}/{}/{}",
        year,
        spring_festival.day.m + 1,
        spring_festival.day.d
    );
