...
impl SolarTerms {
    ...
    pub fn calc(&self, mut jd: f64, is_solar_term: bool) -> f64 {
        ...
        // 平气或平朔
            ...
            let mut i = 0;
            while i < kb.len()-1 {
                if jd + pc < kb[i+2].0 { break; }
                i += 2;
            }
            let mut d = kb[i].0 + kb[i].1 * 
                ((jd + pc - kb[i].0) / kb[i].1).floor();
            d = d.floor() + 0.5;
    ...
    /// 定朔修正表解压数据
    const SUO_S: &'static str = 
        "EqoFscDcrFpmEsF2DfFideFelFpFfFfFiaipqti1ksttikptikqckstekqttgkqttgkqteksttikptikq2fjstgjqtt\
    ...
    /// 定气修正表解压数据 
    const QI_S: &'static str =
        "FrcFs22AFsckF2tsDtFqEtF1posFdFgiFseFtmelpsEfhkF2anmelpFlF1ikrotcnEqEq2FfqmcDsrFor22FgFrcgDsc\
