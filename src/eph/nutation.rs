use crate::eph::eph_base::{rad2mrad,RAD,PI};
...
impl Nutation {
    ...
    pub fn nutation(t: f64, zq: i32) -> [f64; 2] {
        ...
        for i in 0..77 {
            ...
            let c = ( nu_tab_cell.0 as f64 * l +  nu_tab_cell.1 as f64 * l1 +  nu_tab_cell.2 as f64 * f + 
                nu_tab_cell.3 as f64 * d + nu_tab_cell.4 as f64 * om) / RAD;
    
            // 只计算周期大于zq天的项
            if zq > 0 {
                let q = 36526.0 * 2.0 * PI * RAD / 
                        (1717915923.2178* nu_tab_cell.0 as f64 + 129596581.0481* nu_tab_cell.1 as f64 + 
                         1739527262.8478* nu_tab_cell.2 as f64 + 1602961601.2090* nu_tab_cell.3 as f64 + 
            ...
            dl += ( nu_tab_cell.5 as f64 +  nu_tab_cell.6 as f64*t) * c.sin() +  nu_tab_cell.7 as f64 * c.cos();
            de += ( nu_tab_cell.8 as f64 +  nu_tab_cell.9 as f64*t) * c.cos() +  nu_tab_cell.10 as f64 * c.sin();
        }
    ...
    pub fn cd_nutation(z: &[f64], e: f64, dl: f64, de: f64) -> [f64; 3] {
        ...
        // 计算赤经章动
        r[0] += (e.cos() + e.sin() * z[0].sin() * z[1].tan()) * dl 
                - z[0].cos() * z[1].tan() * de;
