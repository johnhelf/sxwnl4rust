...
use crate::eph::eph::{cir_ovl, line_ear, line_ear2, line_ovl};
use crate::eph::eph_base::{
    CS_AU, CS_BA, CS_BA2, CS_K, CS_K0, CS_K2, CS_R_EAR, CS_R_EAR_A, CS_S_MOON, CS_S_MOON2, J2000,
    PI, PI2, RAD, gxc_moon_lat, gxc_moon_lon, gxc_sun_lat, gxc_sun_lon, llr_conv, llr2xyz, mqc,
    p_gst, p_gst2, parallax, rad2mrad, rad2rrad, shi_cha_j, sun_sheng_j, suo_n, xyz2llr,
};
...
impl MSC {
    ...
    pub fn calc(&mut self, t: f64, l: f64, fa: f64, high: f64) {
        ...
        // 计算真恒星时
        self.gst = p_gst(self.jd, self.dt) + self.dl * self.e.cos();
    ...
    fn calc_other_params(&mut self, t: f64) {
        ...
        let lon = (1753470142.0 + 6283319653318.0 * t + 529674.0 * t2 + 432.0 * t3
            - 1124.0 * t4
            - 9.0 * t5)
            / 1.0e9
            + PI
        ...
        let mut lon = rad2mrad(lon - (self.s_cj - self.dl * self.e.cos()));
        if lon > PI {
            lon -= PI2;
        }
        self.sc = lon / PI2;
        ...
        // 计算本影和半影
        self.e_shadow = (CS_R_EAR_A / self.m_r * RAD - (959.63 - 8.794) / self.s_r) * 51.0 / 50.0;
        self.e_shadow2 = (CS_R_EAR_A / self.m_r * RAD + (959.63 + 8.794) / self.s_r) * 51.0 / 50.0;

        // 计算月相照明
        self.m_ill = XL::moon_ill(t);

        // 计算中心食
        if (rad2rrad(self.m_cj - self.s_cj)).abs() < 50.0 / 180.0 * PI {
            let pp = line_ear(
...
impl PosVel {
    pub fn new(xx: f64, yy: f64, tt: f64) -> Self {
        Self {
            x: xx,
            y: yy,
            t: tt,
...
pub fn ec_fast(jd: f64) -> EclipseResult {
    ...
    let mut t3 = t2 * t;
    let t4 = t3 * t;
    let l = (93.2720993 + 483202.0175273 * t - 0.0034029 * t2 - t3 / 3526000.0 + t4 / 863310000.0)
        / 180.0
        * PI;
    ...
    t2 = t * t / 10000.0;
    t3 = t2 * t / 10000.0;
    let mut mb = 18461.0 * (0.0571 + 8433.46616 * t - 0.640 * t2 - 1.0 * t3).cos()
    ...
    // 月亮黄纬速度
    let vb =
        (-755.0 * (0.057 + 8433.4662 * t).sin() - 82.0 * (2.413 + 16762.1576 * t).sin()) / 36525.0;
...
impl RsGS {
    ...
    pub fn init(&mut self, mut jd: f64, n: usize) {
        ...
        for i in 0..n {
            ...
            // 存入插值表
            self.zs
                .extend_from_slice(&[s[0], s[1], s[2], m[0], m[1], m[2]]);
        ...
        // 计算辅助参数
        let p = self.zs.len() - 9;
        self.dyj = (self.zs[2] + self.zs[p + 2] - self.zs[5] - self.zs[p + 5]) / (2.0 * CS_R_EAR);
    ...
    pub fn chazhi(&self, jd: f64, xt: usize) -> Vec<f64> {
        ...
        // 确定插值中心c,并限制范围
        let cc = (t + 0.5).floor() as i32;
        let c;
        if cc <= 0 {
            c = 1;
        }else if cc > (n - 2) as i32 {
            c = (n - 2) as usize;
        } else {
            c = cc as usize;
        ...
        for i in 0..m {
            ...
            z[i] = self.zs[idx]
                + (self.zs[idx + w] - self.zs[idx - w]
                    + (self.zs[idx + w] + self.zs[idx - w] - self.zs[idx] * 2.0) * t)
                    * t
                    / 2.0;
        }
    ...
    pub fn bse_m(&self, jd: f64) -> [f64; 3] {
        let mut a = self.cd2bse(&self.chazhi(jd, 1), &self.chazhi(jd, 2));
        // 转换单位到地球半径
        a[0] /= CS_R_EAR;
        a[1] /= CS_R_EAR;
        a[2] /= CS_R_EAR;
    ...
    pub fn qrd(&self, mut jd: f64, dx: f64, dy: f64, fs: i32) -> [f64; 3] {
        ...
        // 转换为地标系
        let mut re = self.bse2db(&[x / d, y / d, 0.0], &self.bse(jd), true);
    ...
    pub fn feature(&self, jd: f64) -> EclipseFeature {
        ...
        // 计算影轴交点
        let f = line_ear2(ef.xc, ef.yc, 2.0, ef.xc, ef.yc, 0.0, CS_BA, 1.0, &ef.i);
    ...
    fn elm_copy(&self, a: &mut [f64], mut n: i32, b: &[f64], mut m: i32) {
        if b.is_empty() {
            return;
        }
    ...
    pub fn jie_x(&self, jd: f64) -> EclipseLines {
        ...
        for _ in 0..=n {
            ...
            // 处理日出日没食甚线
            if !self.m_dian(&m, vx, vy, false, r, &i, ua) && !ua.is_empty() {
                ua = &mut el.q3;
                
            }
            if !self.m_dian(&m, vx, vy, true, r, &i, ub) && ub.len() > 2 {
                ub = &mut el.q4;
            }
            if t > re.jd {
                if ua.is_empty() {
            ...
            if (p[1] != 100.0 && n4 == 0) || (p[1] == 100.0 && n4 == 1) {
                ...
                // 计算新的贝塞尔参数
                let i2 = vec![i[0], i[1], i[2] - dj / (vx * vx + vy * vy).sqrt() * 6.28];
    ...
    pub fn jie_x2(&self, jd: f64) -> EclipseLines2 {
        ...
        for i in 0..n {
            ...
            // 计算本影界线点
            let (x, y) = (m[0] + b.r2 * cos_s, m[1] + b.r2 * sin_s);
            let mut p = line_ear2(x_sun, y_sun, z, x, y, 0.0, CS_BA, 1.0, &ii);
            ...
            // 计算半影界线点
            let (x, y) = (m[0] + b.r1 * cos_s, m[1] + b.r1 * sin_s);
            p = line_ear2(x_sun, y_sun, z, x, y, 0.0, CS_BA, 1.0, &ii);
...
impl RsPL {
    ...
    pub fn sec_xy(&self, jd: f64, l: f64, fa: f64, high: f64, re: &mut SecXYResult) {
        ...
        // 真恒星时(不考虑非多项式部分)
        let gst = p_gst(jd - deltat, deltat) + zd[0] * (Prece::hcjj(jd / 36525.0) + zd[1]).cos();
        ...
        re.x = rad2rrad(re.m_cj2 - re.s_cj2) * ((re.m_cw2 + re.s_cw2) / 2.0).cos();
    ...
    pub fn sec_max(&mut self, mut jd: f64, l: f64, fa: f64, high: f64) {
        ...
        for i in (-30..30).step_by(6) {
            ...
            self.sec_xy(tt, l, fa, high, &mut g2);
        ...
        if r_min <= g.mr + g.sr {
            ...
            // 计算日出食分
            self.sec_xy(eclipse_data.sun_s, l, fa, high, &mut g2);
            eclipse_data.sf2 = (g2.mr + g2.sr - (g2.x * g2.x + g2.y * g2.y).sqrt()) / g2.sr / 2.0;
            if eclipse_data.sf2 < 0.0 {
                eclipse_data.sf2 = 0.0;
            }

            // 计算日没食分
            self.sec_xy(eclipse_data.sun_j, l, fa, high, &mut g2);
            eclipse_data.sf3 = (g2.mr + g2.sr - (g2.x * g2.x + g2.y * g2.y).sqrt()) / g2.sr / 2.0;
            ...
            for _ in 0..3 {
                self.sec_xy(self.s_t[0], l, fa, high, &mut g2);
            ...
            eclipse_data.v1 =
                rad2mrad(eclipse_data.p1 - shi_cha_j(p_gst2(self.s_t[0]), l, fa, g2.s_cj, g2.s_cw));

            // 计算复圆
            self.s_t[2] = line_t(&ggg, v, u, g.mr + g.sr, true);
            for _ in 0..3 {
                self.sec_xy(self.s_t[2], l, fa, high, &mut g2);
                self.s_t[2] = line_t(&ggg2, v, u, g2.mr + g2.sr, true);
            }

            eclipse_data.p2 = rad2mrad((g2.x / g2.y).atan());
            eclipse_data.v2 =
                rad2mrad(eclipse_data.p2 - shi_cha_j(p_gst2(self.s_t[2]), l, fa, g2.s_cj, g2.s_cw));
        }
        ...
        // 无效时刻置0
        for i in 0..5 {
            if self.s_t[i] < eclipse_data.sun_s || self.s_t[i] > eclipse_data.sun_j {
                self.s_t[i] = 0.0;
            }
        }
    ...
    fn zb0(&mut self, jd: f64) {
        ...
        // 计算t1时刻的参数
        let p = &mut self.p;
        p.g = p_gst(jd - deltat, deltat) + zd[0] * (e + zd[1]).cos(); // 真恒星时(不考虑非多项式部分)
        p.s = rsgs.sun(jd);
        p.m = rsgs.moon(jd);
        ...
        let q = &mut self.q;
        q.g = p_gst(t2 - deltat, deltat) + zd[0] * (e + zd[1]).cos();
        q.s = rsgs.sun(t2);
        q.m = rsgs.moon(t2);
    ...
    pub fn nbj(&mut self, jd: f64) {
        ...
        if self.v[3] != 100.0 && self.v[5] != 100.0 {
            let x = (self.v[2] - self.v[4]) * ((self.v[3] + self.v[5]) / 2.0).cos();
            let y = self.v[3] - self.v[5];
            self.vb = format!("{}千米", (CS_R_EAR_A * (x * x + y * y).sqrt()).round());
        }
    }
}
