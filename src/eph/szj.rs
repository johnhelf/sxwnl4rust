use crate::eph::{
    delta_t::dt_t,
    eph_base::{CS_R_EAR, PI2, RAD, llr_conv, mod2, p_gst, rad2rrad},
    jd::JD,
    prece::Prece,
    xl::XL,
};
...
impl SZJ {
    ...
    pub fn get_h(&self, h: f64, w: f64) -> f64 {
        let c = (h.sin() - self.fa.sin() * w.sin()) / (self.fa.cos() * w.cos());
        if c.abs() > 1.0 { PI } else { c.acos() }
    }
    ...
    pub fn m_coord(&self, jd: f64, h0: bool, r: &mut Coords) {
        ...
        // 升起对应的时角
        if h0 {
            r.h0 = self.get_h(0.7275 * CS_R_EAR / z[2] - 34.0 * 60.0 / RAD, z[1]);
        }
    }
    ...
    pub fn mt(&mut self, mut jd: f64) -> MoonTransit {
        ...
        // 查找最靠近当日中午的月上中天
        jd -= mod2(
            0.1726222 + 0.966136808032357 * jd - 0.0366 * self.dt + self.l / PI2,
            1.0,
        );
