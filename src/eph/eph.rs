...
use crate::eph::eph_base::{CS_AU,CS_BA,CS_BA2,CS_R_EAR,rad2rrad, llr_conv, PI2,CS_XX_HH,CS_GS,p_gst2,rad2mrad,rad2str,parallax,mqc,llr2xyz};
...
pub fn xing_liu(xt: usize, mut t: f64, sn: bool) -> f64 {
    // 先求冲(下合)
    let hh = CS_XX_HH[xt as usize - 1] as f64 / 36525.0;  // 会合周期
    ...
    for _ in 0..6 {
        t -= rad2rrad(
            XL::xl0_calc(xt, 0, t, 8) - XL::xl0_calc(0, 0, t, 8)
        ) / v;
    }
    ...
    let tc = tc[xt as usize - 1] / 36525.0;
...
pub fn xing_hr(xt: usize, mut t: f64, f: bool) -> (f64, f64) {
    ...
    // 计算行星相对地球的黄经平速度
    let mut v = PI2 / CS_XX_HH[xt as usize - 1] as f64 * 36525.0;
    if xt > 2 {
        v = -v;
    }
    ...
    for _ in 0..6 {
        t -= rad2rrad(
            XL::xl0_calc(xt, 0, t, 8) - XL::xl0_calc(0, 0, t, 8) - w0
        ) / v;
    }
...
pub fn xing_x(xt: usize, jd: f64, l: f64, fa: f64) -> String {
    ...
    if xt == 10 { // 月亮计算
        ...
        // 地心黄道及赤道坐标
        z[0] = rad2mrad(z[0] + dl);
        s.push_str(&format!("视黄经 {} 视黄纬 {} 地心距 {}\n",
            rad2str(z[0], 0), rad2str(z[1], 0), format!("{:.1$}", ra, rfn)));

        z = llr_conv(&z, e); // 转到赤道坐标
        s.push_str(&format!("视赤经 {} 视赤纬 {} 光行距 {}\n",
            rad2str(z[0], 1), rad2str(z[1], 0), format!("{:.1$}", rb, rfn)));
    } else if xt < 10 { // 行星和太阳计算
        a = XL::p_coord(0, t, -1, -1, -1);  // 地球
        z = XL::p_coord(xt, t, -1, -1, -1); // 行星
        z[0] = rad2mrad(z[0]);
        s.push_str(&format!("黄经一 {} 黄纬一 {} 向径一 {}\n", 
            rad2str(z[0], 0), rad2str(z[1], 0),format!("{:.1$}", z[2], rfn) ));
    ...
    s.push_str(&format!("站赤经 {} 站赤纬 {} 视距离 {}\n",
        rad2str(z[0], 1), rad2str(z[1], 0), format!("{:.1$}", rc, rfn)));
    ...
    s.push_str(&format!("方位角 {} 高度角 {}\n",
        rad2str(z[0], 0), rad2str(z[1], 0)));
    s.push_str(&format!("恒星时 {}(平) {}(真)\n",
        rad2str(rad2mrad(gst_ping), 1), rad2str(rad2mrad(gst), 1)));
    s
}
...
/// # Returns
/// 交点信息结构体
pub fn line_ell(x1: f64, y1: f64, z1: f64, 
                x2: f64, y2: f64, z2: f64,
                e: f64, r: f64) -> EllPoint {
...
/// # Returns 
/// 交点的经纬度等信息
pub fn line_ear2(x1: f64, y1: f64, z1: f64,
                 x2: f64, y2: f64, z2: f64,
                 e: f64, r: f64, i: &[f64]) -> BesselPoint {
...
pub fn line_ear(p: &[f64], q: &[f64], gst: f64) -> BesselPoint {
    ...
    let r = line_ell(
        ...
        q_xyz[0], q_xyz[1], q_xyz[2], 
        CS_BA, CS_R_EAR 
    );

    let mut bp = BesselPoint::default();
    bp.d = r.d;
